...
import {
  ...
  InserirCapa,
  TextfieldCadastro,
} from "./CadastroForm.styles";
import {
  ContainerGeral,
  VoltarPraHome,
  LinkParaHome,
  SetaEsquerda,
} from "pages/pages.styles";
...
const CadastroForm = () => {
  ...
  React.useEffect(() => {
    ...
    let hoje = new Date();
    let dataHoje: string =
      hoje.getMonth() + 1 + "/" + hoje.getDate() + "/" + hoje.getFullYear();
  ...
  function retornarParaHome() {
    if (
      window.confirm(
        "Tem certeza de que quer cancelar? Você retornará à página principal."
      )
    ) {
      navigate("/home");
    }
    ...
    <ContainerGeral>
      ...
      <CadastroContainer onSubmit={formik.handleSubmit}>
        ...
        <div id="container3">
          ...
          <ButtonMUI id="cadastro-botao-cancelar" onClick={retornarParaHome}>
            cancelar
          </ButtonMUI>
        </div>
      </CadastroContainer>
